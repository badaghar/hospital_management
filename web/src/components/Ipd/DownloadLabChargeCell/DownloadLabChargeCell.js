...
export const QUERY = gql`
  query FindDownloadLabChargeQuery($id: Int!) {
    downloadLabCharge: ipd(id: $id) {
      id
      consultant_doctor
      date_of_admission
      created_at
...
export const Success = ({ downloadLabCharge }) => {
  ...
  useEffect(() => {
    const noOfPage = Math.ceil(downloadLabCharge.IpdLabCharges.length / 15)
    // setPages(noOfPage)
    let page = []
    ...
    const array = downloadLabCharge.IpdLabCharges;
              ...
              >
                  ...
                  <div className="flex f justify-between px-6">
                    <div className="space-x-4">
                      <span className="font-bold text-xs ">Name : </span> <span className="text-xs">
                        {downloadLabCharge.patient.name.split('(')[0]}
                      </span>
                    </div>
                    <div className="space-x-4">
                      <span className="font-bold text-xs">Mobile No : </span> <span className="text-xs">{downloadLabCharge.patient.phone_no}</span>
                    </div>
                    <div className="space-x-4">
                ...
                >
                  ...
                  <div className="grid grid-cols-8  font-bold border-b  text-xs border-black">
                    ...
                    <span className="col-span-5">Name Of Lab Test</span>
                      ...
                      chunks[item].map((lab, ind) => {
                        return (
                          <>
                            ...
                            <span className="col-span-5">{lab.lab_name}</span>
                            <span className="col-span-2">{lab.amount}</span>
                          </>
                        )
