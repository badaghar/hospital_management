...
const SaleMedicineQrcode = (props) => {
  ...
  useEffect(() => {
    if (props.details) {
      ...
      let obj = {
        label: props.details.patient.name,
        value: props.details.patient.id,
      }
      let obj2 = {
        label: props.details.consultant_doctor.split('----')[0],
        value: props.details.consultant_doctor.split('----')[0],
      }
      ...
      let obj3 = props.details.IpdPrescription.map((item) => {
        ...
        let ob = {
          'medicine Name': med.pid.name,
          'batch No': med.batch + ' - ' + med.quantity,
          'Expiry Date':
            med.exp.toString().split('-')[0] +
            '-' +
            med.exp.toString().split('-')[1],
          mrp: med.mrp,
          quantity: item.quantity,
          'cgst/sgst': 0,
          amount: total_amount,
          tax: 0,
          amountWtax: total_amount,
          productId: med.pid.id,
          maxQty: med.quantity,
          batch: med.batch,
        }
        return ob
  ...
  useEffect(() => {
    ...
    const arrDoc = props.users
      .filter((item) => item.roles == 'doctor')
      .map((item) => {
  ...
  const onSubmit = (data) => {
    ...
    for (let i = 0; i < medicineArray.length; i++) {
      ...
      if (seen[key]) {
        console.log(
          `Duplicate found for productId: ${productId} and batch: ${batch}`
        )
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < medicineArray.length; i++) {
      if (
        medicineArray[i]['amount'] == 'NaN' ||
        medicineArray[i]['tax'] == 'NaN' ||
        medicineArray[i]['amountWtax'] == 'NaN'
      ) {
        continue
      }
    ...
    <div className="rw-form-wrapper">
      ...
      <Form onSubmit={onSubmit} error={props.error}>
        ...
        <div className="mt-3 flex items-center  gap-x-4">
          ...
          <Select
            options={compositionToProductLsit}
            isClearable={true}
            placeholder="List Of Medicines"
          />
        </div>

        <div className="w-full bg-white p-2 text-black shadow-sm ">
          <div className=" grid grid-flow-row grid-cols-12 gap-x-2 gap-y-2">
            <div className="col-span-4 flex justify-center">Medicine Name</div>
            <div className="col-span-3 flex justify-center">Batch No</div>
            <div className="col-span-1 flex justify-center">Expiry Date</div>
            <div className="col-span-1 flex justify-center">M.R.P</div>
            <div className="col-span-1 flex justify-center">Quantity</div>
            {/* <div className="flex col-span-1 justify-center">CGST/SGST</div> */}
            <div className="col-span-1 flex justify-center">Amount</div>
            <div className="col-span-1 flex justify-center">Action</div>
          ...
          <div className="mt-2 flex justify-center">
            <div
              className="cursor-pointer rounded-3xl bg-gray-900 p-2 text-white hover:bg-slate-300 hover:text-gray-950"
              onClick={addMedicine}
            >
              Add Medicine
            </div>
        ...
        <div className="w-full bg-white p-2 text-black shadow-sm">
          <div className=" grid grid-flow-row grid-cols-6 gap-x-2 gap-y-2">
            <div className="col-span-4 flex justify-center">Medicine Name</div>
            <div className="col-span-1 flex justify-center">Amount</div>
            <div className="col-span-1 flex justify-center">Action</div>
          ...
          <div className="mt-2 flex justify-center">
            <div
              className="cursor-pointer rounded-3xl bg-gray-900 p-2 text-white hover:bg-slate-300 hover:text-gray-950"
              onClick={addHomoMedicine}
            >
              Add Homopathy Medicine
            </div>
          </div>
        </div>
...
const AddMedicineBody = ({
  ...
  const medicineNameChange = (item) => {
    const exp =
      item.data.exp.toString().split('-')[0] +
      '-' +
      item.data.exp.toString().split('-')[1]
    ...
    setMedicineArray((array) => {
      ...
      newArray[index] = {
        ...newArray[index],
        'medicine Name': item?.data.pid.name || '',
        'batch No': item?.value || '',
    ...
    // let newBatchList = dublicatList.filter(
    //   (it) =>
    //     it.pid.name == item.value && it.quantity > 0 && new Date(it.exp) >= date
    // )
    // console.log(newBatchList)

    // newBatchList = newBatchList.map((it) => {
    //   return {
    //     label: it.batch + ' - ' + it.quantity,
    //     value: it.batch + ' - ' + it.quantity,
    //     batch: it.batch,
    //     id: it.id,
    //     data: it,
    ...
    <>
      <div className=" col-span-7 ">
        <Select
          ...
          value={item['medicine Name'] !== '' ? medicineName : ''}
        />
      </div>
      ...
      <div className="col-span-1 flex justify-center text-black">
        <input
          ...
          required
          onChange={(e) =>
            medicineChange(e.target.name, e.target.value, setQuantity)
          }
        />
      </div>
      ...
      <div className="col-span-1 flex justify-center">
        <span
          className="cursor-pointer p-2 text-xl text-red-600"
          onClick={del.bind(this, index)}
        >
    ...
    <>
      ...
      <div className="col-span-1 flex justify-center text-black">
        <input
          type="number"
          name="amount"
          className="w-24 border border-black p-2"
          placeholder="Amount"
          id=""
          value={amount}
          required
          onChange={(e) =>
            medicineChange(e.target.name, e.target.value, setAmount)
          }
        />
      </div>
...
export default SaleMedicineQrcode
